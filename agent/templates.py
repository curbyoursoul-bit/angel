...
# IMPORTANT:
# - Templates generate strategy files that are ready to run with your engine.
# - The EMA_CROSSOVER template below:
#     * accepts a pandas.DataFrame OR a SmartConnect client
#     * reads --interval/--bars/--symbols from sys.argv (engine CLI)
#     * fetches candles (NSE index -> fallback NFO FUT)
#     * adds debug info when no candles are returned

EMA_CROSSOVER = Template(r'''
from __future__ import annotations
import os, sys
import pandas as pd
...
# The Bollinger+RSI template remains a simple, DataFrame-based signal.
BOLLINGER_RSI = Template(r'''
from __future__ import annotations
import pandas as pd
import numpy as np
...
def _rsi(s: pd.Series, n: int) -> pd.Series:
    ...
    roll_up  = pd.Series(gain, index=s.index).rolling(n).mean()
    roll_dn  = pd.Series(loss, index=s.index).rolling(n).mean()
...
def _signal(df: pd.DataFrame) -> str:
    if "close" not in df.columns or len(df) < max($bb_window, $rsi_window) + 2:
        return "NO_OP"
    _, upper, lower = _bollinger(df["close"], $bb_window, $bb_k)
    ...
    if c < lo and r <= $rsi_buy:
        return "BUY"
    if c > up and r >= $rsi_sell:
        return "SELL"
    return "HOLD"

def run(df: "pd.DataFrame") -> dict | None:
    sig = _signal(df)
    return {
        "name": NAME,
        "meta": META,
