...
from utils.expiry import next_thursday  # consistent with other strategies

from config import (
    DEFAULT_ORDER_TYPE, DEFAULT_PRODUCT, DEFAULT_DURATION,
    ATM_UNDERLYING, ATM_LOTS, ATM_ORDER_VARIETY,
)

# ======= knobs (tweak as needed; uses config defaults) =======
UNDERLYING = ATM_UNDERLYING.upper()   # "NIFTY" | "BANKNIFTY"
LOTS       = int(ATM_LOTS)

BASE_WING_DISTANCE     = 150   # for NIFTY; ~300 for BANKNIFTY
WING_STEP              = 50
...
USE_BS_FAIR_VALUE    = True    # set False to bypass BS checks
RISK_FREE            = 0.065   # ~6.5% India
IV_GUESS             = 0.20    # 20% annual IV
...
EXCHANGE      = "NFO"
ORDER_VARIETY = str(ATM_ORDER_VARIETY or "NORMAL").upper()
PRODUCT_TYPE  = str(DEFAULT_PRODUCT    or "INTRADAY").upper()
DURATION      = str(DEFAULT_DURATION   or "DAY").upper()

STEP_MAP      = {"NIFTY": 50, "BANKNIFTY": 100}
SPOT_TOKEN    = {"NIFTY": "26000", "BANKNIFTY": "26009"}  # NSE index tokens

# ---------- internal Black–Scholes (fallback) ----------
try:
...
def _build(ts: str, tok: str, side: str, ordertype: str, qty: int, limit_price: float | str | None) -> Dict[str, Any]:
    if ordertype == "MARKET":
        price = "0"
    else:
...
def _bs_okay(spot: float, strike: float, dte: int, ltp: float, is_call: bool) -> bool:
    """Check if market LTP is within tolerance of BS fair value."""
    ...
    if not ok:
        ...
        logger.warning(f"BS filter: {kind} K={strike} LTP={ltp:.2f} fair≈{fair:.2f} dev={dev:.0%} > {BS_MAX_MISPRICE_PCT:.0%}")
    return ok
# ------------------------------
...
def _try_wings(
    ...
    """
    Search wings (±dist) until:
      1) BS fair-value sanity passes for all four legs, and
      2) estimated net credit per lot >= MIN_NET_CREDIT_PER_LOT, and
      3) credit >= MIN_CREDIT_OF_RISK_PCT * (dist * lot_size)  (risk-quality guard)
    """
    ...
    if not (_bs_okay(spot, atm, dte, ce_ltp, is_call=True) and _bs_okay(spot, atm, dte, pe_ltp, is_call=False)):
        raise RuntimeError("ATM legs failed BS fairness check; tweak params or disable USE_BS_FAIR_VALUE")
    ...
    for dist in range(BASE_WING_DISTANCE, MAX_WING_DISTANCE + WING_STEP, WING_STEP):
        ...
        # risk quality: credit must be at least some % of structural risk (dist * lot_size)
    ...
    # Fallback to base distance if nothing passed
    if not chosen:
        logger.warning(
            f"No wing distance met credit/BS guards up to ±{MAX_WING_DISTANCE}. "
        ...
        up_ce_ltp  = _safe_option_ltp(smart, "NFO", up_ce_ts,  up_ce_tok)
        low_pe_ltp = _safe_option_ltp(smart, "NFO", low_pe_ts, low_pe_tok)
        # still apply BS guard; if it fails, we proceed but log warning
        if USE_BS_FAIR_VALUE:
            if not _bs_okay(spot, upper, dte, up_ce_ltp, is_call=True):
                logger.warning("Fallback wing CE failed BS fairness check.")
            if not _bs_okay(spot, lower, dte, low_pe_ltp, is_call=False):
                logger.warning("Fallback wing PE failed BS fairness check.")
        net_credit = float("nan")
    else:
        dist, up_ce_ts, up_ce_tok, up_ce_ltp, low_pe_ts, low_pe_tok, low_pe_ltp, net_credit = chosen

    # Build the 4 legs with prices
    qty = lot_size * LOTS
    ordertype = str((DEFAULT_ORDER_TYPE or "LIMIT")).upper()
...
def run(smart) -> List[Dict[str, Any]]:
    ...
    if not isinstance(opts, pd.DataFrame) or opts.empty:
        raise RuntimeError(f"No options data loaded for {und}. Check instruments files.")
    ...
    # 4) Build orders with BS checks + risk-quality filter
    ordertype = str((DEFAULT_ORDER_TYPE or "LIMIT")).upper()
    if ordertype not in ("MARKET", "LIMIT"):
        logger.warning(f"Unexpected DEFAULT_ORDER_TYPE={ordertype!r}; forcing LIMIT")
        ordertype = "LIMIT"
    ...
    # 5) Tag all legs (≤19 chars; Angel cap)
    ordertag = f"IFLY-{und}-{expiry_d.strftime('%d%b').upper()}"[:19]
    for o in orders:
        o["ordertag"] = ordertag
